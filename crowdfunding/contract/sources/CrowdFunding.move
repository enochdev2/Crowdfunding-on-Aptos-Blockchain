...
module crowdfundings::crowdfunding{
    use std::signer;
    use std::vector;
...
///////////////////////////////////////////////
    ...
    public entry fun donate<CoinType>(account: &signer, amount: u64, cf_address: address) acquires Deposit, CrowdFunding{
        assert!(exists<CrowdFunding<CoinType>>(cf_address), CAMPAIGN_DOES_NOT_EXIST);
        ...
        let cf = borrow_global_mut<CrowdFunding<CoinType>>(cf_address); // FFT: What if the address of the cf is passed as an argument?
    ...
    public entry fun claimFunds<CoinType>(account: &signer) acquires Deposit, CrowdFunding{
        ...
        //Checks
        assert!(addr == @crowdfundings, EONLY_CROWDFUNDING_OWNER_CAN_PERFORM_THIS_OPERATION);
        assertGoalReached<CoinType>(true);

        let donors = &mut borrow_global_mut<CrowdFunding<CoinType>>(@crowdfundings).donors;
...
///////////////////////////////////////////////
    ...
    fun assertCrowdfundingInitialized<CoinType>() {
        assert!(exists<CrowdFunding<CoinType>>(@crowdfundings), CAMPAIGN_DOES_NOT_EXIST);
    }

    // Check if deadline has passed
    fun assertDeadlinePassed<CoinType>() acquires CrowdFunding{
        let cf = borrow_global<CrowdFunding<CoinType>>(@crowdfundings);
    ...
    fun withdrawCoinsFromDeposits<CoinType>(donors: &mut vector<address>) acquires Deposit{
        while (!vector::is_empty<address>(donors)){
            ...
            coin::deposit(@crowdfundings, coins);
        }
    }
}
...
// ~ deployment address 0x1d734c9c4abae4da83a98edffd8122a70bc32ea51dd80c6b30a3659d9d03d18d
